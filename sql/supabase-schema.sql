-- ============================================================
-- ENEM STUDY - SCHEMA COMPLETO SUPABASE
-- Plataforma de Estilo Mangá RPG - Black & White
-- ============================================================
...
-- 3.1 Profiles (estende auth.users do Supabase)
...
-- 3.4 Progresso do usuário na jornada
...
-- 3.6 Progresso de vídeos assistidos
...
-- 3.12 Spaced Repetition - fila de revisão
...
-- 3.15 Sessão do usuário na masmorra
CREATE TABLE public.user_dungeon_sessions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  dungeon_id UUID NOT NULL REFERENCES public.dungeons(id) ON DELETE CASCADE,
...
-- 3.16 Resultados por sala da masmorra
CREATE TABLE public.user_dungeon_room_results (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- 3.18 Estatísticas agregadas (cache do grimório)
...
-- 3.19 Sessão persistente (anti-perda de streak)
...
-- 4. ÍNDICES
-- ============================================================
CREATE INDEX idx_profiles_adventurer_name ON public.profiles(adventurer_name);
...
-- 5.2 Auto-criar profile ao registrar
...
-- 5.3 Calcular nível baseado em XP
...
-- 5.4 Atualizar XP e nível do profile
...
-- 5.6 Registrar resposta e aplicar spaced repetition
...
BEGIN
  ...
  FROM public.user_challenge_results
  WHERE user_id = p_user_id AND challenge_id = p_challenge_id;

  INSERT INTO public.user_challenge_results
    (user_id, challenge_id, user_answer, is_correct, time_spent_seconds, xp_earned, attempt_number)
  VALUES
    (p_user_id, p_challenge_id, p_user_answer, v_result, p_time_spent, v_xp, v_attempt);
  ...
  ELSE
    UPDATE public.user_stats
    SET challenges_failed = challenges_failed + 1
    WHERE user_id = p_user_id;
...
-- 5.7 Buscar desafios para revisão (spaced repetition)
CREATE OR REPLACE FUNCTION public.get_review_challenges(
  p_user_id UUID,
...
BEGIN
  ...
  JOIN public.challenges c ON c.id = srq.challenge_id
...
BEGIN
  ...
  CASE p_type
    WHEN 'video' THEN
      UPDATE public.user_stats
      SET total_video_time_seconds = total_video_time_seconds + p_seconds
      WHERE user_id = p_user_id;
    WHEN 'material' THEN
      UPDATE public.user_stats
      SET total_material_time_seconds = total_material_time_seconds + p_seconds
      WHERE user_id = p_user_id;
    WHEN 'challenge' THEN
      UPDATE public.user_stats
      SET total_challenge_time_seconds = total_challenge_time_seconds + p_seconds
      WHERE user_id = p_user_id;
    WHEN 'dungeon' THEN
      UPDATE public.user_stats
      SET total_dungeon_time_seconds = total_dungeon_time_seconds + p_seconds
      WHERE user_id = p_user_id;
  END CASE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
